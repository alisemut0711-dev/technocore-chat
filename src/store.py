    ...
    with _create_gate(
        ...
        if did is not None:
            ...
            if previous is not None and nonce <= previous:
                raise StoreError(
                    f"nonce {nonce} is not greater than {previous}, the highest in the "
                    f"scanned tail of /r/{room} (older writes may lie beyond it) — "
                    f"signed URLs are single-use, count up"
                )
